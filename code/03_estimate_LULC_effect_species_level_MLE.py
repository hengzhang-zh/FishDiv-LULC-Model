# -*- coding: utf-8 -*-
"""
Created on Tue Oct 31 00:40:32 2023
...
#please note that the catchment computing step only runs on a NVIDIA GPU. 
...
if __name__=="__main__":
    ...
    resultfolderdir=root+os.sep+r"result\relation\table\fish_div_LULC_effect_species_level"  #result folder
    ...
    #read species abundance file
    abundfilename="fish_abund.csv"
    abundfiledir=biodivfolderdir+os.sep+abundfilename
    abundPD=init.readCSVasPandas(abundfiledir)
    ...
    decay_func_str="exp"                                                           #exp, sph, matern15, matern25, gaussian
    ...
    #convert species abundance to presence/absence (P/A)
    abundPD=np.array(abundPD[splist],dtype=np.float32)   #select species in the list
    abundPD=(abundPD>1)*1   
...
#%%   
    ...
    C_mat=model.calc_C_mat(ras_LULC,LULC_CVT,biodivPD,siteIDList,catch_file_list,geoTrans_LULC,dist_list,LULC_types,env_var,\
                   num_sites,num_dist_sim,num_LULC_types,decay_func_str,siteloc_thres,bool_sitelocshift,\
                   lon_ori_header,lat_ori_header,lon_calib_header,lat_calib_header)
    
    for i_sp in range(num_sp):
        spname=splist[i_sp]
        print("processing...   sp: %s  (%d / %d)"%(spname,i_sp+1,num_sp))
        biodiv=abundPD[:,i_sp].reshape([num_sites,1])
        LULC_values=model.calc_opt_params_species_level(C_mat,biodiv,dist_list,num_dist_sim,num_LULC_types)        
        #write the results
        if not os.path.exists(resultfolderdir):
            os.makedirs(resultfolderdir)
        csvfilename1="LULC_effect_"+decay_func_str+"_var_"+env_var_name+"_SP"+str(i_sp+1).zfill(3)+"_"+spname+".csv"
        csvfiledir1=resultfolderdir+os.sep+csvfilename1
        init.writeArrayToCSV(LULC_values,["intcp"]+["V_lc"+str(LULC_type) for LULC_type in LULC_types]+["V_env","dist","thres","TSS","metric"],csvfiledir1)
        print("sp: %s is finished. \n"%spname)
    print("ALL DONE.\n")
