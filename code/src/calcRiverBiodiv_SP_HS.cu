...
#define MAX_LOOP 6000
#define MAX_LC_TYPES 12
...
__global__ void calcRiverBiodivSP(float* B, const float* rivPixEnv, const uchar* FD, const uchar* LULC, const float* DX, const float* DY,
	const float* SP_MAT, const int* LC_types, const int decay_func, const int num_LC_types, const int num_sp_mat_cols, const int radius_pix, const int num_rivpix,
	const int num_sp, const int nrow, const int ncol)
{
	...
	if (idx >= num_rivpix)
		...
		dst_point.x = (int)rivPixEnv[idx * 3 + 1];
		dst_point.y = (int)rivPixEnv[idx * 3];
		...
		float flowdist = 0.0;
		//each river pixel: search for local catchment 
		int2 i_start, i_end;
		...
		int i_sim, i_lc;
		float param_vec[MAX_LC_TYPES], LC_values[MAX_LC_TYPES];
			...
			for (i_point.x = i_start.x; i_point.x < i_end.x; i_point.x++) {
				if (((i_point.y - dst_point.y) * (i_point.y - dst_point.y) + (i_point.x - dst_point.x) * (i_point.x - dst_point.x)) > (radius_pix * radius_pix))
					...
					if (abs(flowdist + 1) < 1e-6)  //not in the catchment
						...
						for (i_sim = 0; i_sim < num_sp; i_sim++) {
#pragma unroll
							for (i_lc = 0; i_lc < num_LC_types; i_lc++) {
								param_vec[i_lc] = SP_MAT[INDEX2D_YX(num_sp_mat_cols, i_sim, num_LC_types + 2)];
								LC_values[i_lc] = SP_MAT[INDEX2D_YX(num_sp_mat_cols, i_sim, i_lc)];
							}
							B[INDEX2D_YX(num_sp, idx, i_sim)] += calcContribBiodiv(i_point, flowdist, LULC, DX, DY, decay_func, param_vec, LC_values, LC_types, num_LC_types, nrow, ncol);
						}
					}
				}
			}
...
#pragma unroll
		for (i_sim = 0; i_sim < num_sp; i_sim++) {
			B[INDEX2D_YX(num_sp, idx, i_sim)] +=
				SP_MAT[INDEX2D_YX(num_sp_mat_cols, i_sim, num_LC_types)] +
				SP_MAT[INDEX2D_YX(num_sp_mat_cols, i_sim, num_LC_types + 1)] * rivPixEnv[idx * 3 + 2];
		}
	}
}
